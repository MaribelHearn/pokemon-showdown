...
export const Moves: {[moveid: string]: MoveData} = {
	...
	electricterrain: {
		...
		condition: {
			...
			onFieldStart(field, source, effect) {
				if (effect?.effectType === 'Ability') {
					if (this.format.name.includes('Weather Wars')) this.effectState.duration = 0;
	...
	metronome: {
		...
		flags: {},
		noMetronome: [
			"After You", "Apple Acid", "Assist", "Astral Barrage", "Aura Wheel", "Baneful Bunker", "Beak Blast", "Behemoth Bash", "Behemoth Blade", "Belch", "Bestow", "Body Press",
			"Branch Poke", "Breaking Swipe", "Celebrate", "Chatter", "Clangorous Soul", "Copycat", "Counter", "Covet", "Crafty Shield", "Decorate", "Destiny Bond", "Detect",
			"Diamond Storm", "Double Iron Bash", "Dragon Ascent", "Dragon Energy", "Dragon Hammer", "Drum Beating", "Dynamax Cannon", "Endure", "Eternabeam", "Falcon Punch",
			"False Surrender", "Feint", "Fiery Wrath", "Fleur Cannon", "Focus Punch", "Follow Me", "Freeze Shock", "Freezing Glare", "Glacial Lance", "Grav Apple", "Helping Hand",
			"Hold Hands", "Hyperspace Fury", "Hyperspace Hole", "Ice Burn", "Instruct", "Jungle Healing", "King's Shield", "Life Dew", "Light of Ruin", "Mat Block", "Me First",
			"Meteor Assault", "Metronome", "Mimic", "Mind Blown", "Mirror Coat", "Mirror Move", "Moongeist Beam", "Nature Power", "Nature's Madness", "Obstruct", "Origin Pulse",
	...
	sleeptalk: {
		...
		onHit(pokemon) {
			const noSleepTalk = [
				'assist', 'beakblast', 'belch', 'bide', 'celebrate', 'chatter', 'copycat', 'dynamaxcannon', 'focuspunch', 'mefirst', 'metronome', 'mimic', 'mirrormove', 'naturepower', 'shelltrap', 'sketch', 'sleeptalk', 'uproar', 'falconpunch', 'warlockpunch',
			];
			const moves = [];
			for (const moveSlot of pokemon.moveSlots) {
				const moveid = moveSlot.id;
	...
	falconpunch: {
		...
		condition: {
			duration: 1,
			onStart(pokemon) {
				this.add('-singleturn', pokemon, 'move: Falcon Punch');
			},
			onHit(pokemon, source, move) {
				if (move.category !== 'Status') {
					this.effectState.lostFocus = true;
		...
		beforeMoveCallback(pokemon) {
			if (pokemon.volatiles['focuspunch']?.lostFocus) {
				this.add('cant', pokemon, 'Falcon Punch', 'Falcon Punch');
				return true;
			}
		},
	},
	...
	explod: {
		...
		pp: 5,
		priority: 0,
		flags: {mirror: 1, defrost: 1, bypasssub: 1},
		target: "allAdjacent",
		type: "???",
		selfdestruct: "always",
		willCrit: true,
	...
	dinnerblaster: {
		...
		pp: 15,
		priority: 0,
		flags: {pulse: 1, protect: 1, mirror: 1, allyanim: 1, heal: 1},
		target: "normal",
		type: "Fairy",
		secondary: null,
		onTryHit(target, source, move) {
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				if (move.flags['bullet'] || move.flags['pulse']) {
					...
					this.damage(this.clampIntRange(damage / 2, 1, Math.floor(source.maxhp / 2)), source, source, move);
				}
				return this.NOT_FAIL;
			},
			onHit(target, source, move) {
	...
	nyan: {
		...
		basePowerCallback() {
			if (this.field.pseudoWeather.echoedvoice) {
				return 50 * this.field.pseudoWeather.echoedvoice.multiplier;
			}
			return 50;
		},
		onTry() {
	...
	miraclefruit: {
		...
		pp: 5,
		priority: 0,
		flags: {mirror: 1, snatch: 1, heal: 1},
		target: "self",
		type: "Grass",
		zMove: {effect: 'crit2'},
		heal: [1, 4],
	...
	telemetry: {
		...
		onHit(target, source) {
			...
			if (unrevealedMoves.length > 0) {
				...
				this.add('-activate', source, 'move: Telemetry', target, '[move] ' + move.name);
			}
		},
	},
	delete: {
	...
	denialofservice: {
		...
		target: "normal",
		type: "Electric",
		volatileStatus: 'denialofservice',
		secondary: null,
		condition: {
			// Item suppression implemented in Pokemon.ignoringItem() within sim/pokemon.js
			duration: 5,
	...
	mysterioussong: {
		...
		basePower: 0,
		category: "Status",
		name: "Mysterious Song",
		pp: 5,
		priority: 0,
		flags: {protect: 1, sound: 1, bypasssub: 1, mirror: 1, reflectable: 1},
		target: "normal",
		type: "Normal",
		zMove: {boost: {spd: 1}},
		volatileStatus: 'confusion',
	...
	graspinghands: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, contact: 1, mirror: 1, heal: 1},
		target: "allAdjacent",
		type: "Ghost",
		drain: [1, 2],
		secondary: null,
	...
	takethat: {
		...
		pp: 10,
		priority: -4,
		flags: {contact: 1, protect: 1, mirror: 1, heal: 1, sound: 1, bypasssub: 1},
		target: "normal",
		type: "Fighting",
		drain: [1, 2],
		secondary: null,

export const Items: {[itemid: string]: ItemData} = {
	...
	aguavberry: {
		...
		onUpdate(pokemon) {
			if (pokemon.hp <= pokemon.maxhp / 4 || (pokemon.hp <= pokemon.maxhp / 2 && (pokemon.hasAbility('gluttony') || pokemon.hasAbility('solarglutton')))) {
				pokemon.eatItem();
			}
		},
	...
	custapberry: {
		...
		onFractionalPriority(priority, pokemon) {
			if (
				priority <= 0 &&
				(pokemon.hp <= pokemon.maxhp / 4 || (pokemon.hp <= pokemon.maxhp / 2 && (pokemon.hasAbility('gluttony') || pokemon.hasAbility('solarglutton'))))
			) {
				if (pokemon.eatItem()) {

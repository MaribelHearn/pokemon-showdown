...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	aromaveil: {
		onAllyTryAddVolatile(status, target, source, effect) {
			if (['attract', 'disable', 'encore', 'healblock', 'denialofservice', 'taunt', 'torment'].includes(status.id)) {
	...
	damp: {
		onAnyTryMove(target, source, effect) {
			if (['explosion', 'mindblown', 'mistyexplosion', 'selfdestruct', 'lightcatnarok', 'darkcatnarok', 'fierycatnarok', 'systemcrash'].includes(effect.id)) {
	...
	parentalbond: {
		onPrepareHit(source, target, move) {
			...
			if (['dynamaxcannon', 'endeavor', 'fling', 'iceball', 'rollout', 'ruin', 'clot', 'blab'].includes(move.id)) return;
	...
	corruption: {
		...
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) {
			if (move.corruptionBoosted) return this.chainModify([0x1333, 0x1000]);
		},
		name: "Corruption",
		rating: 4,
		num: 2004,
	...
	cardboardbox: {
		...
		onCriticalHit(target, source, move) {
			if (!target) return;
			if (!['snake'].includes(target.species.id) || target.transformed) {
				return;
			}
    ...
    conflagrate: {
		...
		onModifyType(move, pokemon) {
			...
			if (move.type === 'Normal' && !noModifyType.includes(move.id) && !(move.isZ && move.category !== 'Status')) {
				move.type = 'Fire';
				move.conflagrateBoosted = true;
			}
		},
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) {
			if (move.conflagrateBoosted) return this.chainModify([0x1333, 0x1000]);
		},
		name: "Conflagrate",
		rating: 4,
		num: 2015,
    ...
    gettogether: {
		...
		onResidual(pokemon) {
			if (
				pokemon.baseSpecies.baseSpecies !== 'Boo' || pokemon.level < 20 ||
				pokemon.transformed || !pokemon.hp
			) return;
			if (pokemon.hp > pokemon.maxhp / 4) {
				if (pokemon.species.id === 'boo') {
	...
	taxidermy: {
		...
		onCriticalHit(target, source, move) {
			if (!target) return;
			if (!['chucktesta'].includes(target.species.id) || target.transformed) {
				return;
			}
			const hitSub = target.volatiles['substitute'] && !move.flags['authentic'] && !(move.infiltrates && this.gen >= 6);
			if (hitSub) return;
	...
	pureheart: {
		onTryHit(target, source, move) {
			if (target !== source && move.type === 'Dark') {
				move.accuracy = true;
				if (!target.addVolatile('pureheart')) {
		...
		onEnd(pokemon) {
			pokemon.removeVolatile('pureheart');
		},
		condition: {
			onStart(target) {
				this.add('-start', target, 'ability: Pure Heart');
			},
			onEnd(target) {
				this.add('-end', target, 'ability: Pure Heart', '[silent]');
			},
		},
		name: "Pure Heart",
		rating: 3.5,
	...
	/*squidkid: {
		...
		onBeforeMove(attacker, defender, move) {
			...
			const targetForme = (move.id === 'inkblast' ? 'Inkling-Kid' : 'Inkling-Squid');
			if (attacker.species.name !== targetForme) attacker.formeChange(targetForme);
		},
		name: "Squid Kid",
		rating: 4,
    ...
    trance: {
		onSourceAfterFaint(length, target, source, effect) {
			...
			if (source.species.id === 'terra' && source.hp && !source.transformed && source.side.foe.pokemonLeft) {
				this.add('-activate', source, 'ability: Trance');
				source.formeChange('Terra-Esper', this.effect, true);
			}
		},
		name: "Trance",
		rating: 4,
    ...
    glacialforce: {
		...
		onBasePower(basePower, attacker, defender, move) {
			if (this.field.isWeather('hail')) {
				if (move.type === 'Ice') {
					this.debug('Glacial Force boost');
	...
	reflowering: {
		onAllyModifyAtkPriority: 3,
		onAllyModifyAtk(atk, pokemon) {
			if (this.effectState.target.baseSpecies.baseSpecies !== 'Yuyuko') return;
			if (['sunnyday', 'desolateland'].includes(pokemon.effectiveWeather())) {
				return this.chainModify(1.5);
			}
		},
    ...
    eggwatch: {
		onStart(pokemon) {
			this.add('-ability', pokemon, 'Egg Watch');
		},
		onResidual(pokemon) {

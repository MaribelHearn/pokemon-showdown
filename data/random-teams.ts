...
export class RandomTeams {
	...
	randomFCupTeam(): RandomTeamsTypes.RandomSet[] {
		...
		const randomN = this.randomNFundexPokemon(this.maxTeamSize, this.forceMonotype, 8);
	...
	randomNFundexPokemon(n: number, requiredType?: string, minSourceGen?: number) {
		// Picks `n` random pokemon--no repeats, even among formes
		// Also need to either normalize for formes or select formes at random
		// Fundex only
		const first = 2000;
		...
		for (const species of speciesPool) {
			...
			if (minSourceGen && species.gen < minSourceGen) continue;
			const num = species.num;
			if (num < first || pool.includes(num)) continue;
			if (num > last) break;
			pool.push(num);
		}
	...
	randomNAllPokemon(n: number, requiredType?: string, minSourceGen?: number) {
		// Picks `n` random pokemon--no repeats, even among formes
		// Also need to either normalize for formes or select formes at random
		// Fundex only
		const first = 0;
	...
	randomINTeam(): PokemonSet[] {
		...
		const randomN = this.randomNAllPokemon(this.maxTeamSize, this.forceMonotype, 8);
